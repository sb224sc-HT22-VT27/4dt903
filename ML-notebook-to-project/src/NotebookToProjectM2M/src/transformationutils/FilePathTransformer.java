 ...
 * file paths need to be updated to reference the correct locations: - Simple
 * filenames like 'file.csv' become 'data/file.csv' - Paths like
 */
public class FilePathTransformer {
	...
	/**
	 * Pattern for matching file extensions in paths. Built from
	 * DATA_FILE_EXTENSIONS. Example: "csv|json|yaml|yml|..."
	 */
	 ...
	 * extensions.
	 * 
	 * The pattern uses a backreference (\\1) to ensure opening and closing quotes
	 * match. The character class [^'\"\\n\\r] excludes quotes and newlines to
	 * prevent matching malformed paths or paths containing the opposite quote type.
	 * 
	 * Groups: 1. Opening quote (' or ") 2. Full path (may include directory
	 * components) 3. File extension (without dot)
	 */
	private static final Pattern PATH_PATTERN = Pattern
			.compile("(['\"])([^'\"\\n\\r]*\\.(" + EXTENSIONS_PATTERN + "))\\1", Pattern.CASE_INSENSITIVE);

	/**
	 * Pattern to strip leading './' prefixes from paths. Matches one or more
	 * consecutive './' at the start of the string.
	 */
